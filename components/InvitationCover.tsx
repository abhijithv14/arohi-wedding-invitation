...
export default function InvitationCover({ onOpen }: InvitationCoverProps) {
  ...
  const petals = useMemo(() => Array.from({ length: 22 }, (_, i) => ({ left: `${(i * 47) % 101}%`, size: 6 + (i % 4) * 2, duration: 6 + (i % 5) * 1.4, delay: (i % 8) * 0.45, drift: -55 + (i % 11) * 11, rotate: -120 + (i % 8) * 35 })), []);
  ...
  return <AnimatePresence>{visible && <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.4, ease: "easeInOut" }} className="fixed inset-0 z-[100] overflow-hidden bg-[#2A211D]">
    ...
    {opening && <div className="pointer-events-none fixed inset-0 z-40 overflow-hidden" aria-hidden="true">
      ...
      {petals.map((petal, index) => <motion.span key={index} initial={{ y: "-12vh", x: 0, rotate: petal.rotate, opacity: 0 }} animate={{ y: "112vh", x: petal.drift, rotate: petal.rotate + 280, opacity: [0, 0.72, 0.82, 0.65, 0] }} transition={{ duration: petal.duration, delay: petal.delay, ease: "linear" }} className="absolute top-0 rounded-[70%_30%_70%_30%] bg-[#F5E7DE] shadow-[0_1px_4px_rgba(70,40,25,0.16)]" style={{ left: petal.left, width: petal.size, height: petal.size * 1.55 }} />)}
