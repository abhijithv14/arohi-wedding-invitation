...
export default function InvitationCover({ onOpen }: InvitationCoverProps) {
  ...
  const openInvitation = () => {
    if (opening) return;
    setOpening(true);
    // Keep the existing music contract: this user gesture is the point at which
    // the app is allowed to start/resume its audio. No autoplay is triggered here.
    ...
    >
      ...
      <AnimatePresence>
        {opening && (
          ...
          >
            {/* Warm translucent veil makes the transition feel like a physical invitation curtain. */}
            <motion.div
              initial={{ opacity: 0 }}
            ...
            {petals.map((petal, index) => (
              <motion.span
                ...
                className="absolute top-0 rounded-[70%_30%_70%_30%] bg-[#F5E7DE] shadow-[0_1px_4px_rgba(70,40,25,0.18)]"
                style={{ left: petal.left, width: petal.size, height: petal.size * 1.55 }}
              />
            ))}

            {/* Brief monogram/name reveal before the cover lifts away. */}
            <div className="absolute inset-0 flex items-center justify-center text-center">
              <motion.div
                initial={{ opacity: 0, scale: 0.86, y: 18 }}
                animate={{ opacity: [0, 1, 1, 0], scale: [0.86, 1, 1, 1.03], y: [18, 0, 0, -14] }}
                transition={{ duration: 1.65, times: [0, 0.25, 0.72, 1], ease: "easeInOut" }}
            ...
            {/* The cover itself rises like a curtain. */}
            <motion.div
              initial={{ y: 0 }}
              animate={{ y: "-100%" }}
              transition={{ delay: 1.05, duration: 0.8, ease: [0.65, 0, 0.35, 1] }}
      ...
      <motion.div
        ...
        transition={{ duration: 1.35, delay: opening ? 0.35 : 0, ease: [0.65, 0, 0.35, 1] }}
      ...
      >
        <div className="flex w-full max-w-2xl flex-col items-center justify-center px-8 py-14 sm:px-16 sm:py-20">
          ...
          <motion.div initial={{ opacity: 0, scaleX: .2 }} animate={{ opacity: 1, scaleX: 1 }} transition={{ delay: 1.25, duration: .7 }} className="mx-auto mt-7 mb-7 flex w-full items-center justify-center gap-3">
            <span className="h-px w-10 bg-[#D8A98A]/55" /><span className="text-xs text-[#E7CDBE]">♡</span><span className="h-px w-10 bg-[#D8A98A]/55" />
          </motion.div>

          <motion.h1 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4, duration: .8 }} className="w-full text-center font-serif text-4xl font-light tracking-wide text-[#FFF8F2] sm:text-6xl">
            <span>Aparna</span><span className="mx-3 font-light text-[#D8A98A] sm:mx-5">+</span><span>Rohit</span>
          </motion.h1>

          <motion.button type="button" disabled={opening} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.85, duration: .7 }} onClick={openInvitation} className="mt-12 rounded-full border border-[#F2DED2]/70 bg-[#FFF8F2]/95 px-8 py-3 text-[10px] tracking-[.3em] text-[#5D4034] shadow-2xl transition hover:scale-105 hover:bg-white disabled:pointer-events-none">
            OPEN INVITATION · ♫
          </motion.button>
        </div>
      </motion.div>
